...
public class RMACProfile {
    ...
    public RMACProfile(FabrikController chain, double[]segmentLengthsInches, EffectorPathPlanner path) {
        ...
        for (int i=0;i<path.getInterpolationPositions().size();i++) {
            ...
            TransformationMatrixGenerator armTransformationMatrixInterpolation = new TransformationMatrixGenerator(anglesDegrees, segmentLengthsInches);
        ...
        for (int i=0;i<path.getInterpolationPositions().size()-1;i++) {
            maxAccelerationx.add(maxAcceleration*(transformationMatrices.get(i+1).get(0,3)-transformationMatrices.get(i).get(0,3))/stepSize);
            maxAccelerationy.add(maxAcceleration*(transformationMatrices.get(i+1).get(0,3)-transformationMatrices.get(i).get(1,3))/stepSize);
            maxAccelerationz.add(maxAcceleration*(transformationMatrices.get(i+1).get(0,3)-transformationMatrices.get(i).get(2,3))/stepSize);
        }
        while (!accelerationConstraintResolved) {
            for (int i=0;i<interpolationAccelerations.size()-1;i++) {
                if((path.getInterpolationPositions().get(i+1)[0]-path.getInterpolationPositions().get(i)[0])/Math.pow((interpolationTimes.get(i+1)-interpolationTimes.get(i)),2)>maxAccelerationx.get(i)) {
                interpolationTimes.set(i+1,interpolationTimes.get(i)+Math.max(Math.sqrt((path.getInterpolationPositions().get(i+1)[0]-path.getInterpolationPositions().get(i)[0])/maxAccelerationx.get(i)),Math.sqrt((path.getInterpolationPositions().get(i+1)[1]-path.getInterpolationPositions().get(i)[1])/maxAccelerationz.get(i))));
                operationTime+=Math.max(Math.sqrt((path.getInterpolationPositions().get(i+1)[0]-path.getInterpolationPositions().get(i)[0])/maxAccelerationx.get(i)),Math.sqrt((path.getInterpolationPositions().get(i+1)[1]-path.getInterpolationPositions().get(i)[1])/maxAccelerationz.get(i)));
    ...
    private double findInterpolationTime(double iterations, Float[] interpolationPosition) {
        ...
        for (int i=0;i<iterations;i++) {
            newGuess = oldGuess-(interpolationTimeFunction(oldGuess,startToInterpolationPosition)/interpolationTimeFunctionDerivative(oldGuess, startToInterpolationPosition));
            oldGuess = newGuess;
        }
        return newGuess;
    }
    private double interpolationTimeFunction(double value,float startToInterpolationPosition) {
        return (2*startToInterpolationPosition/maxVelocity)+(operationTime*Math.sin(2*Math.PI*value/operationTime)/2*Math.PI)-value;
    }
    private double interpolationTimeFunctionDerivative(double value, float startToInterpolationPosition) {
        return (2*startToInterpolationPosition/maxVelocity)+(operationTime*Math.cos(2*Math.PI*value/operationTime))-1;
    }
